...
export default function Reportes() {
  ...
  const exportInventory = () => {
    ...
    downloadCSV(
      invReport.products.map((p) => ({
        ...
        "Valor Stock": p.stock_value,
        Estado: p.low_stock ? "Bajo Stock" : "OK",
      })),
      "inventario.csv",
    );
  };
  ...
  const exportSales = () => {
    ...
    downloadCSV(
      salesReport.top_products.map((p) => ({
        ...
        SKU: p.product_sku,
        "Cantidad Vendida": p.total_quantity,
  ...
  const salesDates = (salesReport?.sales_by_date || []).map((d) =>
    new Date(d.period).toLocaleDateString("es-CR", {
      day: "2-digit",
      month: "short",
    }),
  );
  const salesTotals = (salesReport?.sales_by_date || []).map((d) =>
    parseFloat(d.total),
  );
  const salesCounts = (salesReport?.sales_by_date || []).map((d) => d.count);

  const catNames = (invReport?.by_category || []).map((c) => c.category_name);
  const catValues = (invReport?.by_category || []).map((c) =>
    parseFloat(c.total_value),
  );
    ...
    <>
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        <StatCard
          ...
          value={fmt(dash?.stock_value)}
          iconBg="#d4eddf"
        />
        <StatCard
          icon={<FiAlertTriangle />}
          label="Bajo Stock"
          value={dash?.low_stock_count ?? 0}
          iconBg="#fde8e6"
        />
        <StatCard
          icon={<FiFileText />}
          label="Ventas Hoy"
          value={fmt(dash?.sales_today?.total)}
          sub={`${dash?.sales_today?.count ?? 0} transacciones`}
          iconBg="#fef3c7"
        />
      </div>
              ...
              {(invReport?.products || []).map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    {p.category_name ? (
                      <span
                        className="badge"
                        style={{
                          background: p.category_color + "22",
                  ...
                  <td
                    style={{
                      fontWeight: 700,
                      color: p.low_stock ? "var(--red)" : "inherit",
                    }}
                  >
                    {p.stock}
                  </td>
                  ...
                  <td>
                    <span
                      className={`badge ${p.low_stock ? "badge-red" : "badge-green"}`}
                    >
                      {p.low_stock ? (
                        <>
